...
func _physics_process(delta: float) -> void:
	...
	if recoil_data:
		...
		weapon_node.rotation = _current_rotation + _rotational_recoil_offset + _sway_rotation + Vector3(0.0, 0.0, _bob_roll)
		...
		weapon_node.rotation = _current_rotation + _sway_rotation + Vector3(0.0, 0.0, _bob_roll)
...
func apply_bob(offset: Vector3, roll: float):
	_bob_offset = offset
	_bob_roll = roll
